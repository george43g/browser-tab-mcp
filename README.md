...
| `plan_tab_change` | Plan ONE transform (move/setOrder/reverse/sort/pack/act) over a selection — or a declarative `endState` naming windows and the leading run of tabs each should hold (§11: same-domain entries move live; cross-domain entries declare transport, `auto` never cuts; copy/cut halves return as prepared requests). Nothing is applied: ordered primitive effects, warnings, risk class, and a short-lived `planId` for `apply_tab_layout`. Policy violations fail at plan time with stable codes. CLI: `browser-tab plan [--transform\|--end-state]`. | read-only |
| `apply_tab_layout` | Apply a live-layout plan: state-preserving moves only, never reloads or closures. Not transactional — per-effect outcomes, actual final arrangements, residual reported honestly. Stale plans refused by default; `conflict:"replan"` re-plans the same members (budget 1), `"best-effort"` applies what still holds. Every execution lands in the operation journal with a §15 undo record (`browser-tab operations`). A timeout means the outcome is **unknown, not failed**: the daemon finishes the plan on its own, so check the journal before re-planning. CLI: `browser-tab apply --plan <id> [--conflict <mode>]`. | |
| `apply_destructive_plan` | Apply a **destructive** act plan (`discard`/`reload`) across a selection; requires `confirmDestruction:true`. A separate tool rather than a flag on `apply_tab_layout`, because that tool is annotated non-destructive and a flag would make the annotation false exactly when it matters (§26.2). Stale plans are refused outright and never re-planned — re-deriving a discard against a changed world is a person's decision. The journal records what was LOST, not a restore path. CLI: `browser-tab apply-destructive --confirm-destruction`. | destructive |
...
| `closed_tabs` | Tabs the daemon watched disappear, newest first — url, title, where they were, whether the window went too, and the group identity that dies with a group's last member. Reopenable for a bounded window (6h by default). Cross-window moves, browser quits and authority switches are NOT closures and never appear. CLI: `browser-tab closed`. | read-only, idempotent |
| `reopen_tab` | Reopen one of them. Prefers the browser's own recently-closed entry, which restores back/forward history; falls back to opening the recorded URL at its old position, which does not — and the result says **which**, rather than leaving you to find out by pressing Back. A whole-window restore is never taken on a request for one tab. CLI: `browser-tab reopen --id <id>`. | open-world |
