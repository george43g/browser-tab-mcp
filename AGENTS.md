...
12. content + screenshot tools under `BROWSER_TAB_FAKE_ADAPTER=1`: `get_page` / `annotate` / `screenshot` error cleanly (all daemon/extension-only), and `screenshot` with neither/both ids is schema-rejected
