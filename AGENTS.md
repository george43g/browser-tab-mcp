...
- **THERE IS NO `macos-latest` CI LEG, AND RE-ADDING ONE IS A BILLING DECISION.** GitHub bills macOS at **10x** Linux on a private repo. Measured 2026-08-18: 38 macOS jobs in one day = 131 billable minutes = **$10.48**, which was **71% of that day's CI spend on 28% of its minutes**. The jobs were healthy (3.2 min median, no stalls) — the cost was volume x multiplier. It also bought almost nothing: every platform gate in this suite is `process.platform !== "win32"` (POSIX), never `=== "darwin"`, and the macOS-only production paths are driven from any runner via `BROWSER_TAB_PLATFORM` + fakes. The one genuinely Darwin-only artifact — rust-accel's CoreGraphics half — is covered by `pnpm verify:macos` on the developer's own Mac, which is *stricter* than the runner was (it loads and calls the binary; the runner only compiled it). Lint/typecheck/coverage/npm-pack are Linux-only for the same reason: one answer, paid for once.
