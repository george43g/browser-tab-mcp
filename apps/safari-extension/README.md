...
### The window declares itself; it never earns a window-manager rule

The window is **fixed-size and not `.resizable`**, and that is load-bearing. A
fixed-size window is how a macOS app tells a tiling window manager to leave it
alone — it is the platform's Accessibility contract (`AXStandardWindow`), not a
yabai feature, so it works for Amethyst and AeroSpace too. An earlier revision
added `.resizable` merely to grow the stock 425×325 window; yabai promptly took
it into the tiling tree at **1996×1269**, on top of whatever was underneath.
Measured before and after with one variable changed:

| | frame | `is-floating` | `can-resize` | `stack-index` |
|---|---|---|---|---|
| with `.resizable` | 1996×1269 | `false` | `true` | 2 |
| without | 460×387 | `true` | `false` | 0 |

The control: another standard window on the same space stayed tiled
(`kitty`, 1996 wide) across both, so this is yabai ignoring *our* window, not
yabai being off.

**An app we own DECLARES what it is; it never earns a rule.** A yabai rule in
`~/dotfiles/wm-stack` for one of our own binaries is a smell — it means we
shipped a window that misrepresents itself and then corrected it somewhere
invisible to anyone reading the app's source. Rules are for third-party apps
that cannot declare.

Fixed-size would normally mean clipping, because the error strings are
variable-length. Instead the page reports `document.body.scrollHeight` over the
existing `controller` bridge after every render and the window sizes itself
(clamped 300–760pt, top edge pinned). Never resizable, never clipped, never
tiled. `.utilityWindow` / `AXFloatingWindow` is the other way to declare, but it
does nothing on a plain `NSWindow` — it needs an `NSPanel`, and the storyboard
here is regenerated by `convert.sh`.
