...
cmd select help="Resolve a control-language selector against the live snapshot (needs daemon)" {
    flag --selector help="Selector AST as JSON; @<file> reads a file, - reads stdin" required=#true {
        arg <json>
    }
    flag --projection help="Result shape" {
        arg "[p]" required=#false default=core {
...
cmd closed help="Recently closed tabs the daemon remembers" {
    flag --browser help="Only this browser\'s closures" {
        arg <name>
    }
    flag --limit help="How many recent closures to list" {
