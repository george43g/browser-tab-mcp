...
```
browser-tab mcp   → runMcpServer({ transport: 'stdio' | 'http' })
browser-tab tui   → dynamic import of src/tui/index.tsx → runTui()
browser-tab repl  → readline loop → program.parseAsync per line
browser-tab <tool> → callMcpTool(toolName, argsFromFlags)
```
